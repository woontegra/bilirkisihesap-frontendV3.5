/**
 * Tanıklı Standart — UBGT takvim seçici.
 * Seçilen günler ExclusionItem {type, start, end, days} olarak uygulanır.
 */
  ...
  const apply = () => {
    ...
    const ubgtItems: ExclusionItem[] = Array.from(selected)
      ...
      .map((date) => {
        ...
        const days = entry && entry.days > 0 ? entry.days : 1;
        ...
        {invalidRange ? (
          ...
          <>
            <p className={styles.modalDesc}>
              Aşağıda, seçilen döneme ait UBGT günleri listelenir. İşaretlediğiniz günler, işçinin çalışmadığı UBGT
              günleri olarak kabul edilir ve fazla mesai hesabında dışlanır.
            </p>
                    ...
                    {group.entries.map((entry) => (
                      <li key={entry.date}>
                        <label className={styles.ubgtDayLabel}>
                          ...
                          <span className={styles.ubgtDayName}>— {entry.label}</span>
                        </label>
                      </li>
                    ))}
                  </ul>
