/**
 * Yeraltı İşçisi Fazla Mesai — istisna (dışlama) paneli. Yıllık İzin / Rapor /
 * Diğer / UBGT / Puantaj-Bordro kayıtlarını start/end/days modeliyle yönetir;
 * setleri lokal olarak kaydedip başka bir hesaplamaya aktarabilir. Yalnızca UBGT
 * ve Yıllık İzin türleri fazla mesai hesabını etkiler (motor tarafında
 * filtrelenir); diğerleri kayıt/gösterim amaçlıdır.
 */
      ...
      {showSaveModal ? (
        <div className={styles.modalOverlay} role="presentation" onClick={() => setShowSaveModal(false)}>
          <div className={styles.modalCard} role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()}>
            ...
            <p className={styles.modalDesc}></p>
            <input
