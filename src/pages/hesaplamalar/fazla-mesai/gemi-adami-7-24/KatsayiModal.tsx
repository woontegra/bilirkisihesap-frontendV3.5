    ...
    <div className={styles.modalOverlay} role="presentation" onClick={onClose}>
      <div className={styles.modalCard} role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()}>
        ...
        <p className={styles.modalDesc}>
          Bilinen ücretin asgari ücrete oranı katsayı olarak uygulanır; cetveldeki tüm dönem brütleri bu katsayı ile
          çarpılır.
        </p>
        <div className={styles.basicGrid}>
          <label className={styles.field}>
            <span className={styles.fieldLabel}>Bilinen Ücret (₺)</span>
            <input className={styles.dateInput} inputMode="decimal" value={bilinenUcret} onChange={(e) => setBilinenUcret(sanitizeMoneyTyping(e.target.value))} placeholder="0,00" />
          </label>
          <label className={styles.field}>
            <span className={styles.fieldLabel}>Asgari Ücret (₺)</span>
            <input className={styles.dateInput} inputMode="decimal" value={asgariUcret} onChange={(e) => setAsgariUcret(sanitizeMoneyTyping(e.target.value))} placeholder="0,00" />
          </label>
        </div>
        ...
        <div className={styles.modalActions}>
          {currentKatsayi !== 1 && currentKatsayi > 0 ? (
            <Button variant="danger" onClick={() => { onReset(); onClose(); }}>Sıfırla (1)</Button>
          ) : null}
