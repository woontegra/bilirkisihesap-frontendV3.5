...
export function MahsuplasamaModal({ open, years, onSave, onClose }: {
  open: boolean;
  years: number[];
  ...
  return (
    <div className={styles.modalOverlay} role="presentation" onClick={onClose}>
      <div className={`${styles.modalCard} ${styles.modalCardWide}`} role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()}>
        ...
        {years.length === 0 ? (
          <p className={styles.emptyText}>Hesaplama cetvelinde dönem bulunamadı. Önce tarihleri girin.</p>
        ) : (
            ...
            <table className={styles.dataTable}>
              ...
              <tbody>
                {MONTH_NAMES.map((name, idx) => (
                  <tr key={name}><td>{name}</td>{years.map((year) => (
                    <td key={year}><input className={styles.cellInput} inputMode="decimal" value={values[`${year}-${idx + 1}`] ?? ""} onChange={(e) => setCell(year, idx + 1, e.target.value)} placeholder="0" aria-label={`${name} ${year} mahsup tutarı`} /></td>
                  ))}</tr>
                ))}
              </tbody>
            </table>
        ...
        <div className={styles.modalActions}>
          ...
          <Button variant="primary" disabled={years.length === 0} onClick={() => { onSave(total); onClose(); }}>Uygula</Button>
        </div>
      </div>
    </div>
  );
