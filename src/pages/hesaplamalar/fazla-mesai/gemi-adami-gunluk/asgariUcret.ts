/**
 * Standart Fazla Mesai — asgari ücret tablosu (1996–2026), yalnızca bu
 * klasöre ait yerel kopya. Diğer fazla mesai alt türleri veya kıdem/davacı
 * motorlarıyla paylaşılmaz; her modül kendi tablosunu taşır.
 *
...
/**
 * Dönem SINIRLARI (bölme sınırları) — bilirkişi motorundaki `dateSegmentationCore`
 * ile birebir aynıdır ve ÜCRET tablosundan (yukarıdaki `ASGARI_UCRET_TABLOSU`)
 * KASITLI OLARAK farklıdır: 2015 burada TEK tam yıl segmentidir (ücret tablosunda
 * 2015 iki yarım yıla bölünmüş olsa da, satır/hafta bölme sınırı farklıdır — bu
 * segmentin ücreti, segment başlangıcına göre `getAsgariUcretByDate` ile ayrıca
 * okunur). Yalnızca 2007–2014 ve 2022–2023 yıl ortasında (Ocak–Haziran /
 * Temmuz–Aralık) bölünür; diğer tüm yıllar (2015 dahil) tam yıl tek segmenttir.
 */
...
export type AsgariSplitSegment = {
  ...
  /** Segmentin başlangıç tarihine göre ücret tablosundan okunan brüt tutar. */
  brut: number;
  /** Bu segmentin bölünme sınırı tam yıl mı (52 hafta üst sınırı) yoksa yarım yıl mı (26 hafta). */
  fullYear: boolean;
};

/**
 * [startISO, endISO] aralığını dönem SINIRLARINA (bölme motoru) göre alt
 * segmentlere böler. Her segmentin ücreti, segmentin başlangıç tarihine göre
 * `ASGARI_UCRET_TABLOSU`'ndan (ücret dönemleri) okunur — bölme sınırları ile
 * ücret dönemleri kasıtlı olarak ayrı iki kaynaktır (bkz. dosya başı not).
 */
...
/** Tanık/davacı segment pivotları için asgari dönem kesişimleri. */
export function getAsgariUcretPeriodsInRange(
  startISO: string,
  endISO: string,
): Array<{ start: string; end: string; brut: number }> {
  return splitRangeByAsgariPeriodBounds(startISO, endISO).map((s) => ({
    start: s.start,
    end: s.end,
    brut: s.brut,
  }));
