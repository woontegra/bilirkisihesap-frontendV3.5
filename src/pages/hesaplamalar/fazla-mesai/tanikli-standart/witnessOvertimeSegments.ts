/**
 * V3 `buildMergedWitnessSegments` — birebir kopya (izolasyon).
 * Örtüşmede daha geç başlayan tanık kazanır; aynı günde listede sonra gelen kazanır.
 * Tanık kapsamayan dilimde satır üretilmez (davacı fill YOK).
 */
...
export function buildMergedWitnessSegments(
  ...
  for (const seg of segments) {
    ...
    const isAdjacent =
      !!last && new Date(last.end).getTime() + DAY_MS === new Date(seg.start).getTime();
    if (
      last &&
      isAdjacent &&
