/**
 * Tanıklı Standart Fazla Mesai — lokal depolama. Hesaplama kayıtları yalnızca
 * bu tarayıcıda, sayfaya özel bir anahtar altında tutulur. Backend isteği
 * yoktur.
 */

import type { SavedCase, TanikliFormSnapshot } from "./model";
import { newLocalId } from "./model";

export const TANIKLI_FM_CASES_KEY = "bilirkisi-hesap-v35:fm-tanikli:cases:v1" as const;
...
export function loadCasesSafe(): LoadResult<SavedCase> {
  return safeRead<SavedCase>(TANIKLI_FM_CASES_KEY, (parsed) => {
...
function writeCases(cases: SavedCase[]): void {
  ...
  localStorage.setItem(TANIKLI_FM_CASES_KEY, JSON.stringify(payload));
}
...
export function clearCorruptCases(): void {
  if (typeof window === "undefined") return;
  localStorage.removeItem(TANIKLI_FM_CASES_KEY);
}
