/**
 * Fazla Mesai — 48 Saat Vardiya — lokal depolama.
 * Kayıtlar yalnızca bu tarayıcıda saklanır; backend/API çağrısı yoktur.
 * Anahtar vardiya-24 ile tamamen ayrıdır.
 */

import type { Vardiya48FormSnapshot, SavedCase } from "./model";
import { newLocalId } from "./model";

export const VARDIYA48_CASES_KEY = "bilirkisi-hesap-v35:fm-vardiya-48:cases:v1" as const;
...
export function loadCasesSafe(): LoadResult<SavedCase> {
  return safeRead<SavedCase>(VARDIYA48_CASES_KEY, (parsed) => {
...
function writeCases(cases: SavedCase[]): void {
  ...
  localStorage.setItem(VARDIYA48_CASES_KEY, JSON.stringify(payload));
}

export function saveCase(
  name: string,
  form: Vardiya48FormSnapshot,
  result: { toplamFm: number; haftaSayisi: number },
...
export function clearCorruptCases(): void {
  if (typeof window === "undefined") return;
  localStorage.removeItem(VARDIYA48_CASES_KEY);
}
