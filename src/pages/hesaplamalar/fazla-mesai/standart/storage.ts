/**
 * Standart Fazla Mesai — lokal depolama. Hesaplama kayıtları yalnızca bu
 * tarayıcıda, sayfaya özel bir anahtar altında tutulur. Backend isteği yoktur.
 */

import type { SavedCase, StandartFormSnapshot } from "./model";
import { newLocalId } from "./model";

/** v2: model şeması değişti (start/end/days dışlama, zamanaşımı, satır düzeltmeleri, toplam bazlı brütten-nete). Eski v1 kayıtları okunmaz; kullanıcı yeniden kaydetmelidir. */
export const STANDART_FM_CASES_KEY = "bilirkisi-hesap-v35:fm-standart:cases:v2" as const;
...
export function loadCasesSafe(): LoadResult<SavedCase> {
  return safeRead<SavedCase>(STANDART_FM_CASES_KEY, (parsed) => {
...
function writeCases(cases: SavedCase[]): void {
  ...
  localStorage.setItem(STANDART_FM_CASES_KEY, JSON.stringify(payload));
}
...
export function clearCorruptCases(): void {
  if (typeof window === "undefined") return;
  localStorage.removeItem(STANDART_FM_CASES_KEY);
}
