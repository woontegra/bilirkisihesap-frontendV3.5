/**
 * Dönemsel Haftalık Fazla Mesai — hesaplama motoru (V3 pipeline, izolasyon).
 *
 * Sıra: tanık clamp → intervals → asgari split → sezon split → weeks=round(gün/7)
 * → zamanaşımı → expandRows → calculateFm → deduction FM override → preserveWeeks
 * → para → overrides/manual → 270 → net.
 */
...
import {
  annualLeaveMetaFromHaftalikPattern,
  buildIntervalsFromWitnesses,
  calcFmHoursPerWeekHaftalik,
  calculateDonemselHaftalikDeductionFmHours,
  clampDonemselWitnessesToDavaci,
  weeklyIgnoredWeekdayFromHaftalikPattern,
} from "./seasonalHours";
import {
  newLocalId,
  type DonemselHaftalikFormSnapshot,
  type DonemselHaftalikResult,
...
/** V3: weeks = Math.round(diffDays / 7). */
...
function applyDeductionFmOverride(row: PeriodRow): PeriodRow {
  const meta = row.donemselHaftalikDeduction;
...
/** Yaz/kış segmentlere böl (V3 seasonSegmentRow haftalik). */
function seasonSegmentRow(
  ...
  let segSeason = monthSeason(cur.getMonth() + 1);
  ...
  while (cur <= rowEnd) {
    ...
    if (changed || lastDay) {
      ...
      const fmHours = calcFmHoursPerWeekHaftalik(pattern);
      const brut = getAsgariUcretByDate(siso) || 0;
      const leaveMeta = annualLeaveMetaFromHaftalikPattern(pattern);
      const annualLeaveWeeklyIgnoredWeekday = weeklyIgnoredWeekdayFromHaftalikPattern(pattern);
...
export function buildDonemselHaftalikRows(params: {
  dateIn: string;
  dateOut: string;
  summerPattern: SeasonalHaftalikPattern;
  winterPattern: SeasonalHaftalikPattern;
  ...
  const intervals = buildIntervalsFromWitnesses(dateIn, dateOut, witnessesClamped);
  ...
  for (const interval of intervals) {
    ...
    for (const seg of segments) {
      ...
      const brut = getAsgariUcretByDate(siso) || 0;
      const month = new Date(siso).getMonth() + 1;
      ...
      const seasonRows = seasonSegmentRow(baseRow, summerMonths, activeSummerP, activeWinterP, kats);
      rawRows.push(...seasonRows);
    }
  }
...
function applyZamanasimi(rows: PeriodRow[], nihaiBaslangic: string | null): PeriodRow[] {
  if (!nihaiBaslangic) return rows;
  return rows
    .map((r) => {
      if (!r.startISO || !r.endISO) return r;
      if (r.endISO < nihaiBaslangic) return null;
      if (r.startISO < nihaiBaslangic && r.endISO >= nihaiBaslangic) {
        const adjWeeks = calculateWeeksBetweenDates(nihaiBaslangic, r.endISO);
        return {
          ...r,
          startISO: nihaiBaslangic,
          weeks: adjWeeks,
...
export function computeDonemselHaftalikResult(
  ...
  const yazFmHours = calcFmHoursPerWeekHaftalik(form.summerPattern);
  const kisFmHours = calcFmHoursPerWeekHaftalik(form.winterPattern);
  ...
  let rows = buildDonemselHaftalikRows({
    dateIn: form.dateIn,
  ...
  rows = applyZamanasimi(rows, zamanasimiBaslangic);

  const weeklyOffDay = weeklyIgnoredWeekdayFromHaftalikPattern(form.summerPattern);
  const originalTotalWeeks = rows.reduce(
    (a, r) => a + Math.max(0, Math.floor(Number(r.weeks) || 0)),
    0,
  );

  rows = expandRowsForDeductions({
    rows,
    ...
    seasonalDeductionContext: {
      summerPattern: form.summerPattern,
      winterPattern: form.winterPattern,
      summerMonths: form.summerPattern.months ?? [],
    },
  });

  rows = rows.map((r) => calculateFm(r));
  rows = rows.map((r) => applyDeductionFmOverride(r));
  rows = preserveWeeks(rows, originalTotalWeeks);
  rows = rows.map((r) => ({
    ...r,
    katsayi: katSayi,
    fm: calcRowFm(r.brut, katSayi, r.weeks, r.fmHours),
  }));
  ...
  if (form.mode270 === "simple") {
    displayRows = displayRows.map((r) => {
      ...
      return { ...r, fmHours, fm: calcRowFm(r.brut, r.katsayi || katSayi, r.weeks, fmHours) };
    });
  } else if (form.mode270 === "detailed") {
    displayRows = apply270RuleFrontend(displayRows).map((r) => ({
      ...r,
  ...
  const toplamFm = round2(visibleRows.reduce((sum, r) => sum + (r.fm || 0), 0));
  ...
  const mahsupTutari = parseMoneyInput(form.mahsup);
...
export { formatDateTR, calcFmHoursPerWeekHaftalik, countWeeksBySevenDaySteps };
