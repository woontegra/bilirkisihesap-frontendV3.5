/**
 * Kötü Niyet Tazminatı — versiyonlu lokal depo.
 */

import type { ExtraItem, KotuNiyetForm, SavedCase } from "./model";
import { createEmptyForm, newLocalId } from "./model";

export const STORAGE_KEY = "bilirkisi-hesap-v35:kotu-niyet-tazminati:cases:v1" as const;
...
function normalizeForm(raw: Partial<KotuNiyetForm> | undefined | null): KotuNiyetForm {
    ...
    const items = parsed.cases
      ...
      .map((c) => ({
        ...
        results: {
          ...
          weeks: Number(c.results?.weeks) || 0,
        },
      }));
