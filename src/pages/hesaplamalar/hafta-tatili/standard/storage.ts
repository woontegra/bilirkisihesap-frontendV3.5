...
import { createEmptyForm, type SavedCase, type StandardForm } from "./model";

export const STORAGE_KEY = "bilirkisi-hesap-v35:hafta-tatili-standard:cases:v1" as const;
...
function normalizeRanges(raw: unknown): DateRange[] {
  ...
  return raw
    .map((x) => {
      ...
      return {
        id: String(rec.id || newLocalId("dr")),
...
function normalizeForm(raw: Partial<StandardForm> | undefined | null): StandardForm {
  const empty = createEmptyForm();
  if (!raw || typeof raw !== "object") return empty;
  const kg = Number(raw.kullanimGunSayisi);
  const kullanimGunSayisi = (kg === 1 || kg === 2 || kg === 3 || kg === 4 ? kg : 4) as 1 | 2 | 3 | 4;
  return {
    ...
    kullanimBaslangic: String(raw.kullanimBaslangic ?? ""),
    kullanimBitis: String(raw.kullanimBitis ?? ""),
    kullanimGunSayisi,
