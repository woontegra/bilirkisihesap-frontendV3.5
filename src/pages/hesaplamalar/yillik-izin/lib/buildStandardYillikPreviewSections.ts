...
import type { StandardComputeResult, StandardYillikFormBase, UsedLeaveRow } from "./types";
...
function validUsedRows(rows: UsedLeaveRow[]): UsedLeaveRow[] {
  return rows.filter((r) => r.start && r.end && String(r.days ?? "").trim().length > 0);
}

export function buildStandardYillikPreviewSections(opts: {
  form: StandardYillikFormBase;
