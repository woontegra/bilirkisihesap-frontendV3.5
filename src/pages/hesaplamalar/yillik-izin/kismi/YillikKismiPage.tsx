...
import { buildKismiYillikPreviewSections } from "../lib/buildKismiYillikPreviewSections";
...
import { kismiYillikSaveGate, yillikKismiBackend } from "./backendCase";
import { clampYear, computeYillikKismiResult } from "./engine";
...
const PAGE_TITLE = "Yıllık Ücretli İzin — Kısmi Süreli";
const PREVIEW_TITLE = "Yıllık İzin — Kısmi Süreli Raporu";

const backendConfig = makeYillikBackendConfig({
  backend: yillikKismiBackend,
  createEmptyForm,
  snapshotKey,
  loadCasesSafe,
  deleteCase,
  clearCorruptCases,
  getSavePayload: kismiYillikSaveGate,
});

export default function YillikKismiPage() {
  ...
  const previewSections = useMemo(
    () => buildKismiYillikPreviewSections({ form: syncedForm, result }),
    [syncedForm, result],
  );
  ...
  return (
    <YillikPageView
      ...
      previewContentId="yillik-kismi-preview-content"
      notes={NOTE_BLOCKS}
      startDate={syncedForm.startDate}
      endDate={syncedForm.endDate}
      ...
      workPeriodsSlot={
        <WorkPeriodsEditor
          title="Çalışma dönemleri (kısmi süreli)"
      ...
      {...usedRowHandlers}
      usedLeaveSetsModuleId="yillik-izin-kismi-used-leave"
