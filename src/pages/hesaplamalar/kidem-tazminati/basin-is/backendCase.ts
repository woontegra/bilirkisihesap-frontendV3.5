...
import { newLocalId, type BasinFormSnapshot, type ExtraItem, type SavedCase } from "./model";

export { KIDEM_BASIN_TYPE };
...
export function mapBasinFormFromBackend(
  data: unknown,
  record?: Pick<SavedCaseRecord, "ise_giris" | "isten_cikis">,
): BasinFormSnapshot | null {
  try {
    const payload = unwrapData(data);
    const form = pickForm(payload);
    return formatKidemMoneyFields({
      meslegeBaslangic: normalizeDateInput(form.meslegeBaslangic),
      ...
      denemeSuresiGun: str(form.denemeSuresiGun),
...
export function buildBasinSaveResult(brut: number, net: number): CalcSaveResult {
  return { brut, net };
}
