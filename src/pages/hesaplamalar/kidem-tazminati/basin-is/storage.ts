/**
 * Kıdem Tazminatı — Basın İş — lokal depolama.
 * Kayıtlar yalnızca bu tarayıcıda saklanır; backend/API çağrısı yoktur.
 */

import type { BasinFormSnapshot, SavedCase } from "./model";
import { newLocalId } from "./model";

export const BASIN_CASES_KEY = "bilirkisi-hesap-v35:kidem-basin:cases:v1" as const;
...
export function loadCasesSafe(): LoadResult<SavedCase> {
  return safeRead<SavedCase>(BASIN_CASES_KEY, (parsed) => {
...
function writeCases(cases: SavedCase[]): void {
  ...
  localStorage.setItem(BASIN_CASES_KEY, JSON.stringify(payload));
}
...
export function clearCorruptCases(): void {
  if (typeof window === "undefined") return;
  localStorage.removeItem(BASIN_CASES_KEY);
}
