/**
 * Kıdem Tazminatı — Kısmi Süreli / Part Time — kıdem tazminatı tavan tutarları.
 *
 * Bu veri, bu modüle özel bağımsız bir kopyadır (izolasyon kuralı — cross-type
 * engine import yasak). Kaynak: frontendV3/src/calculations/kidem-tazminati/utils.ts
 * (KIDEM_TAVAN_DONEMLERI) ve aktuerya-backend/src/utils/findKidemTavan.js ile aynı
 * dönem/tutar eşlemesi kullanılır.
 */
...
/** Verilen çıkış tarihine karşılık gelen kıdem tazminatı tavanını döndürür (yoksa null). */
export function findKidemTavan(exitDate: Date): number | null {
  ...
  for (const d of KIDEM_TAVAN_DONEMLERI) {
    const start = parseTRDate(d.start);
    const end = parseTRDate(d.end);
    if (normalized >= start && normalized <= end) return d.tavan;
  }
  return null;
}
