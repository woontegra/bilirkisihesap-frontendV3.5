/**
 * Kıdem Tazminatı — Kısmi Süreli / Part Time — lokal depolama.
 * Kayıtlar yalnızca bu tarayıcıda saklanır; backend/API çağrısı yoktur.
 */

import type { KismiFormSnapshot, SavedCase } from "./model";
import { newLocalId } from "./model";

export const KISMI_CASES_KEY = "bilirkisi-hesap-v35:kidem-kismi:cases:v1" as const;
...
export function loadCasesSafe(): LoadResult<SavedCase> {
  return safeRead<SavedCase>(KISMI_CASES_KEY, (parsed) => {
...
function writeCases(cases: SavedCase[]): void {
  ...
  localStorage.setItem(KISMI_CASES_KEY, JSON.stringify(payload));
}

export function saveCase(
  name: string,
  form: KismiFormSnapshot,
  results: { brut: number; net: number },
...
export function clearCorruptCases(): void {
  if (typeof window === "undefined") return;
  localStorage.removeItem(KISMI_CASES_KEY);
}
