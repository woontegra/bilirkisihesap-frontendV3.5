/**
 * Gemi Adamları — kıdem tazminatı tavan tablosu (v1).
...
export const GEMI_KIDEM_TAVAN_DONEMLERI: TavanDonemi[] = [
...
export function findGemiTavan(exitDate: Date): number | null {
  const normalized = new Date(exitDate.getFullYear(), exitDate.getMonth(), exitDate.getDate());
  for (const donem of GEMI_KIDEM_TAVAN_DONEMLERI) {
