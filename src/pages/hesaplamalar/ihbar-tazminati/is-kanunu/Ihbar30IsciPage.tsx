...
import { clampYear, computeIhbar30IsciResult, formatDateTR, formatMoney, isDateOrderInvalid, parseNum } from "./engine";

import { createEmptyForm, snapshotKey, type Ihbar30IsciForm } from "./model";

import { NOTE_BLOCKS } from "./notes";

import { clearCorruptCases, deleteCase, loadCasesSafe } from "./storage";

import styles from "./Ihbar30IsciPage.module.css";



const PAGE_TITLE = "İhbar Tazminatı — İş Kanununa Göre";

const PREVIEW_TITLE = "İhbar Tazminatı — İş Kanununa Göre Raporu";



const backendConfig = makeIhbarBackendConfig({

  backend: ihbar30Backend,
...
export default function Ihbar30IsciPage() {

  const { error: showError } = useToast();

  const [form, setForm] = useState<Ihbar30IsciForm>(createEmptyForm);
  ...
  const applyLoadedForm = useCallback((loaded: Ihbar30IsciForm) => {
  ...
  const patch = useCallback(<K extends keyof Ihbar30IsciForm>(key: K, value: Ihbar30IsciForm[K]) => {
  ...
  return (
    ...
    <div className={styles.wrap}>
      ...
      <IhbarPageView
        ...
        pageDescription="4857 sayılı İş Kanunu kapsamında ihbar süresi (2/4/6/8 hafta) ve tazminat hesabı."
