...
import { ihbarKismiBackend } from "../lib/ihbarBackendInstances";
...
import { clampYear, computeIhbarKismiResult, formatDateTR, formatMoney, isDateOrderInvalid, parseNum } from "./engine";
import { createEmptyForm, snapshotKey, type IhbarKismiForm } from "./model";
import { NOTE_BLOCKS } from "./notes";
import { clearCorruptCases, deleteCase, loadCasesSafe } from "./storage";
import styles from "./IhbarKismiPage.module.css";

const PAGE_TITLE = "İhbar Tazminatı — Kısmi Süreli İş Sözleşmesi";
const PREVIEW_TITLE = "İhbar Tazminatı — Kısmi Süreli Raporu";

const backendConfig = makeIhbarBackendConfig({
  backend: ihbarKismiBackend,
  createEmptyForm,
  snapshotKey,
  loadCasesSafe,
  deleteCase,
...
export default function IhbarKismiPage() {
  ...
  const [form, setForm] = useState<IhbarKismiForm>(createEmptyForm);
  ...
  const applyLoadedForm = useCallback((loaded: IhbarKismiForm) => {
    setForm(loaded);
  ...
  return (
    <div className={styles.wrap}>
      <IhbarPageView
        ...
        pageDescription="Kısmi süreli iş sözleşmelerinde ihbar süresi (2/4/6/8 hafta) ve tazminat hesabı. Çalışma süresi kapsayıcı olmayan (exclusive) takvim farkıyla hesaplanır."
        icon={Clock3}
