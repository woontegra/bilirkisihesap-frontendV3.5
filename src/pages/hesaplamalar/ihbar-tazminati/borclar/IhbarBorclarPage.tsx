...
import { ihbarBorclarBackend } from "../lib/ihbarBackendInstances";
...
import { clampYear, computeIhbarBorclarResult, formatDateTR, formatMoney, isDateOrderInvalid, parseNum } from "./engine";

import { createEmptyForm, snapshotKey, type IhbarBorclarForm } from "./model";

import { NOTE_BLOCKS } from "./notes";

import { clearCorruptCases, deleteCase, loadCasesSafe } from "./storage";

import styles from "./IhbarBorclarPage.module.css";



const PAGE_TITLE = "İhbar Tazminatı — Borçlar Kanunu";

const PREVIEW_TITLE = "İhbar Tazminatı — Borçlar Kanunu Raporu";



const backendConfig = makeIhbarBackendConfig({

  backend: ihbarBorclarBackend,
...
export default function IhbarBorclarPage() {
  ...
  const [form, setForm] = useState<IhbarBorclarForm>(createEmptyForm);
  ...
  const applyLoadedForm = useCallback((loaded: IhbarBorclarForm) => {
  ...
  const patch = useCallback(<K extends keyof IhbarBorclarForm>(key: K, value: IhbarBorclarForm[K]) => {
  ...
  return (
    ...
    <div className={styles.wrap}>
      ...
      <IhbarPageView
        ...
        pageDescription="Türk Borçlar Kanunu kapsamındaki iş ilişkilerinde ihbar süresi (2/4/6/8 hafta) ve tazminat hesabı."
