/**
 * İhbar Tazminatı — gelir vergisi (ücretliler) kademeli tarife tablosu, 2010–2030.
 * `fazla-mesai/standart/incomeTax.ts` dosyasının yerel bir kopyasıdır (değerler resmî GİB
 * tebliğleriyle uyumludur); yalnızca ihbar-tazminati modülü içinde paylaşılır ve başka bir
...
export const INCOME_TAX_BRACKETS: Record<number, IncomeTaxBracket[]> = {
  2030: [
...
export type IncomeTaxResult = {
  tax: number;
  brackets: { rate: number; ratePercent: string; taxableAmount: number; taxAmount: number }[];
  /** Örn. "(%15, %20, %27)" */
  summary: string;
};

/** FrontendV3 `calculateIncomeTax` — tablo baseTax + (matrah - baseLimit) * rate. */
export function calculateIncomeTaxRaw(year: number, income: number): number {
  const brackets = INCOME_TAX_BRACKETS[year];
  if (!brackets || !Number.isFinite(income) || income <= 0) return 0;
...
/** FrontendV3 `calculateIncomeTaxForLumpSum` — ihbar/kıdem tek seferlik matrah. */
export function calculateIncomeTaxForLumpSum(year: number, matrah: number): number {
  return Math.round(calculateIncomeTaxRaw(year, matrah) * 100) / 100;
}
 ...
 * Not: `tax` alanı V3 lump-sum (baseTax) ile üretilir; dilim özeti bilgilendirme amaçlıdır. */
export function calculateIncomeTaxWithBrackets(year: number, income: number): IncomeTaxResult {
  ...
  return { tax: calculateIncomeTaxForLumpSum(year, income), brackets: applied, summary };
}
