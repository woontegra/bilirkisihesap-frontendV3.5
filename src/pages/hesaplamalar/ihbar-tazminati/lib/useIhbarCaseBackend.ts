/**
 * İhbar Tazminatı — backend kayıt yükleme/kaydetme/silme (tüm varyantlar).
 */
...
export type UseIhbarCaseBackendConfig<TForm, TSaved extends { id: string; name: string; form: TForm }> = {
  ...
  getSavePayload: (result: { toplamBrut: number }) => { ok: true } | { ok: false; message: string };
};

export function useIhbarCaseBackend<TForm, TSaved extends { id: string; name: string; form: TForm }>(
  config: UseIhbarCaseBackendConfig<TForm, TSaved>,
  ...
  return {
    ...
    onConfirmDelete,
    clearStorageError,
    backendLoadedCaseIdRef,
  };
}

export type IhbarComputeResult = {
  toplamBrut: number;
  brut: number;
  gelirVergisi: number;
  damgaVergisi: number;
...
export function defaultIhbarSaveGate(result: { toplamBrut: number }): { ok: true } | { ok: false; message: string } {
  if (!(result.toplamBrut > 0)) {
    return { ok: false, message: "Geçerli bir brüt ücret giriniz" };
  }
  return { ok: true };
}
