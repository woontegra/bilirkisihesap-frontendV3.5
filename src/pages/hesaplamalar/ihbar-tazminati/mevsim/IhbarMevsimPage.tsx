...
import { ihbarMevsimBackend } from "../lib/ihbarBackendInstances";
...
import { clampYear, computeIhbarMevsimResult, formatDateTR, formatMoney, isDateOrderInvalid, parseNum } from "./engine";
import { createEmptyForm, snapshotKey, type IhbarMevsimForm } from "./model";
import { NOTE_BLOCKS } from "./notes";
import { clearCorruptCases, deleteCase, loadCasesSafe } from "./storage";
import styles from "./IhbarMevsimPage.module.css";

const PAGE_TITLE = "İhbar Tazminatı — Mevsimlik İşçi";
const PREVIEW_TITLE = "İhbar Tazminatı — Mevsimlik İşçi Raporu";

const backendConfig = makeIhbarBackendConfig({
  backend: ihbarMevsimBackend,
  createEmptyForm,
  snapshotKey,
  loadCasesSafe,
  deleteCase,
...
export default function IhbarMevsimPage() {
  ...
  const [form, setForm] = useState<IhbarMevsimForm>(createEmptyForm);
  ...
  const applyLoadedForm = useCallback((loaded: IhbarMevsimForm) => {
    setForm(loaded);
  ...
  const result = useMemo(() => computeIhbarMevsimResult(form), [form]);
  const dirty = snapshotKey(form) !== baseline;

  const patch = useCallback(<K extends keyof IhbarMevsimForm>(key: K, value: IhbarMevsimForm[K]) => {
  ...
  return (
    <div className={styles.wrap}>
      <IhbarPageView
        ...
        pageDescription="Mevsimlik iş sözleşmelerinin belirsiz süreliye dönüşmesi halinde ihbar süresi (2/4/6/8 hafta) ve tazminat hesabı."
        icon={Sun}
