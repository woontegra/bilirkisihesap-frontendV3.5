/**
 * Davacı Ücreti — Kaydedilmiş Ekstra Hesaplama Setleri API katmanı.
 *
 ...
 * Başarı yalnızca backend cevabıyla kabul edilir.
 */
export async function upsertExtraSet(name: string, items: ExtraItem[]): Promise<SavedExtraSet> {
  ...
  if (!normalized) {
    // Backend 200 dönüp gövde beklenmedikse yeniden listelemeden önce isimle eşleştirilemez;
    // çağıran taraf listExtraSets ile yenileyecek.
    return {
      id: 0,
      name: trimmed,
      data,
